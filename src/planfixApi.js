...
module.exports = {
  ...
  async init() {
    /* this.client = new Planfix({
      privateKey: config.planfix.private_key,
      apiKey: config.planfix.api_key,
    }); */
  },
  ...
  async ensureAuthenticated() {
    if (!this.sid) {
      const result = await this.request('auth.login', {
        login: config.planfix.user_login,
        password: config.planfix.user_password,
      });
  ...
  async request(method, opts) {
    ...
    var requestObject = {
      ...{
        $: { method: method },
        account: config.planfix.account,
      },
      ...opts,
    };
    ...
    const answer = await axios.request({
      ...
      responseType: 'text',
      auth: {
        username: config.planfix.api_key,
  ...
  getContactUrl(generalId) {
    return `https://${config.planfix.account}.planfix.ru/contact/${generalId}`;
  },
};
